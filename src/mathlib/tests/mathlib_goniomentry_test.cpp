...
TEST(SIN, suite){
    EXPECT_DOUBLE_EQ(Sine(0), 0);
    EXPECT_DOUBLE_EQ(Sine(M_PI), 0);
...
TEST(COSINE, suite){
    ...
    EXPECT_DOUBLE_EQ(Cosine(M_PI), -1);
    EXPECT_DOUBLE_EQ(Cosine(M_PI/2), 0);
    EXPECT_DOUBLE_EQ(Cosine(3*M_PI/2), 0);
    EXPECT_TRUE(Cosine(2)<= -0.41614 && Cosine(2)> -0.41615);
}

TEST(TANGENT, suite){
    EXPECT_DOUBLE_EQ(Tangent(0), 0);
    EXPECT_DOUBLE_EQ(Tangent(M_PI), 0);
